...
class Cinema:
    def __init__(self, name: str, location: str, screen_count: int) -> None:
        self.name = name
    ...
    def get_schedule(self) -> List[MovieShowtime]:
        movie_showtimes = []
        for screen in self.screens:
            movie_showtimes.extend(screen.get_schedule())
        return movie_showtimes

    # sourcery skip: use-next
    def get_screen_by_num(self, screen_num: int) -> Optional[Screen]:
        return next(
            (screen for screen in self.screens if screen.screen_num == screen_num),
            None,
        )

    def get_movies_on_screen(self, screen_num: int) -> List[MovieShowtime]:
        screen = self.get_screen_by_num(screen_num)
        return [ms.MovieShowtime for ms in screen.movie_showtimes] if screen else []
...
class Ticket:
    def __init__(self, ticket_num, seat_num, movie_showtime, customer):
        ...
        print(
            f"Ticket {self.ticket_num} for {self.movie_showtime.movie.title} purchased by {self.customer.name}")
